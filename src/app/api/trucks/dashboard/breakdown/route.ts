...
import { getDashboardBreakdown } from "@/actions/dashboard";
import { parsePeriodFromParams, periodIncludesToday } from "@/actions/dashboard/helpers";
import { ALLOWED_GROUP_BY, type BreakdownGroupBy } from "@/types/dashboard";
...
export async function GET(req: NextRequest) {
  ...
  // TODO (SDN-141): add "origen" and "destino" to ALLOWED_GROUP_BY when the module is ready
  if (!ALLOWED_GROUP_BY.includes(groupBy as BreakdownGroupBy)) {
    return NextResponse.json(
      { error: `groupBy inválido. Valores aceptados: ${ALLOWED_GROUP_BY.join(", ")}` },
      { status: 400 }
    );
  }
  ...
  try {
    const data = await getDashboardBreakdown(frente, period, groupBy as BreakdownGroupBy);
